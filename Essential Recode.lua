...
-- [UPDATED] Clean Config System + Smart Pallet ESP (No Boxes)
...
-- АНИМИРОВАННАЯ ЗВЕЗДА РЯДОМ С НАЗВАНИЕМ
...
local function ShouldBeSearchable(obj)
    local current = obj
    while current and current ~= game do
        if current.Name == "ConfigPage" then
            return false
        end
...
local function CreateToggle(parent, text, default, callback)
    ...
    if ShouldBeSearchable(parent) then
...
CreateButton(ConfigPage, "Reload Configs", BgElement, function()
    RefreshConfigList()
    StatusLabel("Status: Config list reloaded!")
end)
...
-- ============================================================
-- СКАНИРОВАНИЕ КАРТЫ (УЛУЧШЕННАЯ ФИЛЬТРАЦИЯ ПАЛЛЕТОК)
-- ============================================================
...
print("[Essential Recode]: Script Loaded Successfully! (Smart Pallet ESP)")