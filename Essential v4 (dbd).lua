...
ScreenGui.IgnoreGuiInset = true -- Важно для прицела
...
table.insert(connections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.RightShift then
        ...
        if isMenuOpen then
            MainFrame.Visible = true MenuScale.Scale = 0.85
...
TabPadding.PaddingTop = UDim.new(0, 12) TabPadding.PaddingLeft = UDim.new(0, 10) TabPadding.PaddingRight = UDim.new(0, 10)
...
ContentPadding.PaddingTop = UDim.new(0, 12) ContentPadding.PaddingLeft = UDim.new(0, 12) ContentPadding.PaddingRight = UDim.new(0, 12) ContentPadding.PaddingBottom = UDim.new(0, 12)

-- ФУНКЦИИ БИБЛИОТЕКИ UI
local tabs = {}
local function CreateTab(name)
    local btn = Instance.new("TextButton", TabContainer)
    btn.Size = UDim2.new(1, 0, 0, 32) btn.BackgroundColor3 = AccentColor btn.BackgroundTransparency = 1 btn.Text = name btn.TextColor3 = TextMuted btn.TextSize = 13 btn.Font = Enum.Font.GothamSemibold
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)

    local page = Instance.new("ScrollingFrame", ContentArea)
    page.Size = UDim2.new(1, 0, 1, 0) page.BackgroundTransparency = 1 page.ScrollBarThickness = 3 page.ScrollBarImageColor3 = Color3.fromRGB(60, 60, 65) page.BorderSizePixel = 0 page.Visible = false
    
    local layout = Instance.new("UIListLayout", page)
    layout.SortOrder = Enum.SortOrder.LayoutOrder layout.Padding = UDim.new(0, 8)
    layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() page.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 5) end)
...
local function CreateToggle(parent, text, default, callback)
    ...
    local stroke = Instance.new("UIStroke", frame) stroke.Color = Color3.fromRGB(40, 40, 42) stroke.Thickness = 1
    
    local label = Instance.new("TextLabel", frame)
    label.Size = UDim2.new(1, -70, 1, 0) label.Position = UDim2.new(0, 12, 0, 0) label.BackgroundTransparency = 1 label.Text = text label.TextColor3 = TextColor label.TextSize = 13 label.Font = Enum.Font.Gotham label.TextXAlignment = Enum.TextXAlignment.Left
    
    local btn = Instance.new("TextButton", frame)
    btn.Size = UDim2.new(0, 36, 0, 18) btn.Position = UDim2.new(1, -48, 0.5, -9) btn.BackgroundColor3 = default and AccentColor or Color3.fromRGB(45, 45, 48) btn.Text = ""
    Instance.new("UICorner", btn).CornerRadius = UDim.new(1, 0)
    
    local circle = Instance.new("Frame", btn)
    circle.Size = UDim2.new(0, 12, 0, 12) circle.Position = UDim2.new(0, default and 21 or 3, 0.5, -6) circle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
...
local function CreateSlider(parent, text, min, max, default, callback)
    ...
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6) Instance.new("UIStroke", frame).Color = Color3.fromRGB(40, 40, 42)
    
    local label = Instance.new("TextLabel", frame)
    label.Size = UDim2.new(1, -20, 0, 24) label.Position = UDim2.new(0, 12, 0, 5) label.BackgroundTransparency = 1 label.Text = text label.TextColor3 = TextColor label.TextSize = 13 label.Font = Enum.Font.Gotham label.TextXAlignment = Enum.TextXAlignment.Left
    
    local valLabel = Instance.new("TextLabel", frame)
    valLabel.Size = UDim2.new(0, 38, 0, 18) valLabel.Position = UDim2.new(1, -50, 0, 8) valLabel.BackgroundColor3 = BgDark valLabel.Text = tostring(default) valLabel.TextColor3 = Color3.fromRGB(200, 200, 205) valLabel.TextSize = 11 valLabel.Font = Enum.Font.Gotham
    Instance.new("UICorner", valLabel).CornerRadius = UDim.new(0, 4)
    
    local bar = Instance.new("Frame", frame)
    bar.Size = UDim2.new(1, -24, 0, 5) bar.Position = UDim2.new(0, 12, 0, 38) bar.BackgroundColor3 = BgDark
    Instance.new("UICorner", bar).CornerRadius = UDim.new(1, 0)
    
    local fill = Instance.new("Frame", bar)
    fill.Size = UDim2.new((default - min)/(max - min), 0, 1, 0) fill.BackgroundColor3 = AccentColor
...
local function CreateButton(parent, text, bgCol, callback)
    ...
    btn.Size = UDim2.new(1, 0, 0, 36) btn.BackgroundColor3 = bgCol or BgElement btn.Text = text btn.TextColor3 = Color3.fromRGB(255, 255, 255) btn.TextSize = 13 btn.Font = Enum.Font.GothamSemibold
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6) Instance.new("UIStroke", btn).Color = Color3.fromRGB(40, 40, 42)
...
local function CreateLabel(parent, defaultText)
    ...
    label.Size = UDim2.new(1, 0, 0, 20) label.BackgroundTransparency = 1 label.Text = defaultText label.TextColor3 = TextMuted label.TextSize = 12 label.Font = Enum.Font.Gotham label.TextXAlignment = Enum.TextXAlignment.Center
...
local function CreateDropdownSection(parent, titleText)
    ...
    Instance.new("UICorner", mainFrame).CornerRadius = UDim.new(0, 6) Instance.new("UIStroke", mainFrame).Color = Color3.fromRGB(40, 40, 42)
    ...
    triggerBtn.MouseButton1Click:Connect(function()
        ...
        arrow.Text = isOpen and "▲" or "▼" arrow.TextColor3 = isOpen and AccentColor or TextColor
    end)
...
local function CreateDropdown(parent, text, options, default, callback)
    ...
    local label = Instance.new("TextLabel", frame)
    label.Size = UDim2.new(0.5, 0, 0, 42) label.Position = UDim2.new(0, 12, 0, 0) label.BackgroundTransparency = 1 label.Text = text label.TextColor3 = TextColor label.TextSize = 13 label.Font = Enum.Font.Gotham label.TextXAlignment = Enum.TextXAlignment.Left

    local selector = Instance.new("TextButton", frame)
    selector.Size = UDim2.new(0, 130, 0, 26) selector.Position = UDim2.new(1, -142, 0, 8) selector.BackgroundColor3 = BgDark selector.Text = "  " .. tostring(default) selector.TextColor3 = Color3.fromRGB(200, 200, 205) selector.TextSize = 12 selector.Font = Enum.Font.GothamSemibold selector.TextXAlignment = Enum.TextXAlignment.Left
    ...
    for _, opt in ipairs(options) do
        ...
        optBtn.MouseEnter:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = AccentColor}):Play() end)
        optBtn.MouseLeave:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = BgDark}):Play() end)
...
local function BuildEssentialHUD()
    ...
    hudContainer = Instance.new("Frame") hudContainer.Name = "EssentialHUD" hudContainer.AnchorPoint = Vector2.new(0, 0) hudContainer.Position = UDim2.new(0, 16, 0, 16) hudContainer.BackgroundColor3 = BgMain hudContainer.BorderSizePixel = 0 hudContainer.AutomaticSize = Enum.AutomaticSize.XY hudContainer.ClipsDescendants = true
    Instance.new("UICorner", hudContainer).CornerRadius = UDim.new(0, 8) Instance.new("UIStroke", hudContainer).Color = Color3.fromRGB(35, 35, 38)
    ...
    hudTitle = Instance.new("TextLabel", hudContainer) hudTitle.LayoutOrder = 1 hudTitle.Size = UDim2.new(0, 130, 0, 18) hudTitle.BackgroundTransparency = 1 hudTitle.Text = "Essential" hudTitle.TextColor3 = Color3.fromRGB(255, 255, 255) hudTitle.TextSize = 13 hudTitle.Font = Enum.Font.GothamBold hudTitle.TextXAlignment = Enum.TextXAlignment.Left hudTitle.AutomaticSize = Enum.AutomaticSize.X
    hudFPS = Instance.new("TextLabel", hudContainer) hudFPS.LayoutOrder = 2 hudFPS.Size = UDim2.new(0, 130, 0, 18) hudFPS.BackgroundTransparency = 1 hudFPS.Text = "FPS: --" hudFPS.TextColor3 = TextColor hudFPS.TextSize = 12 hudFPS.Font = Enum.Font.GothamSemibold hudFPS.TextXAlignment = Enum.TextXAlignment.Left hudFPS.AutomaticSize = Enum.AutomaticSize.X
    hudPing = Instance.new("TextLabel", hudContainer) hudPing.LayoutOrder = 3 hudPing.Size = UDim2.new(0, 130, 0, 18) hudPing.BackgroundTransparency = 1 hudPing.Text = "Ping: -- ms" hudPing.TextColor3 = TextColor hudPing.TextSize = 12 hudPing.Font = Enum.Font.GothamSemibold hudPing.TextXAlignment = Enum.TextXAlignment.Left hudPing.AutomaticSize = Enum.AutomaticSize.X
    hudKiller = Instance.new("TextLabel", hudContainer) hudKiller.LayoutOrder = 4 hudKiller.Size = UDim2.new(0, 130, 0, 18) hudKiller.BackgroundTransparency = 1 hudKiller.Text = "Killer: Unknown" hudKiller.TextColor3 = TextColor hudKiller.TextSize = 12 hudKiller.Font = Enum.Font.GothamSemibold hudKiller.TextXAlignment = Enum.TextXAlignment.Left hudKiller.AutomaticSize = Enum.AutomaticSize.X
...
sep.Size = UDim2.new(1, 0, 0, 1) sep.BackgroundColor3 = Color3.fromRGB(35, 35, 38) sep.BorderSizePixel = 0
...
print("[Essential]: Script Loaded Successfully! (+Fixed Aspect Ratio)")